...
function opensrsemail_ConfigOptions() {
    $config = array(
        ...
        "Cluster"    => array("Type" => "dropdown", "Options" => "A,B,Test", "Description" => "Select the cluster associated with your account",),
...
function opensrsemail_CreateAccount($params) {
    $openSRS = new openSRS_mail($params["configoption1"], $params["configoption2"], $params["configoption3"]);
    try {
...
function opensrsemail_mailbox($params) {
    ...
    if(!isset($_POST["modaction"])) {
        if(isset($_GET["modaction"]) && $_GET["modaction"] == "workgroup") {
            $this->vars["error"] = array("Managing Workgroups is not supported with this module.");
...
class opensrsemail_Controller {
    ...
    public function addEditMailbox($params) {
        ...
        try {
            ...
            if($type == "mailbox" ) {
                ...
                $rs =  mysql_query("SELECT domain from tblhosting WHERE userid = '".$params['clientsdetails']['userid']."' AND packageid = '".$params['packageid']."' and domainstatus = 'Active'");
                while ($row = mysql_fetch_array($rs)){
                   $domains[]['alias_domain'] = $row['domain'] ;
                }
                $result = $openSRS->getDomainMailboxes($params["domain"]);
                ...
                if($result["is_success"]) {
                    ...
                    $this->vars["alias_domains"] = $domains;
                    $this->vars["mailboxes"] = $mailboxes;
    ...
    public function saveMailbox($params) {
        ...
        if($this->getVar("new")) {
            ...
            {
                  $this->vars["error"][] = "Maximum 60 characters are allowed for Title. (ex. Ms. or Mr.)";
            }
            elseif(strlen($this->getVar("firstName")) > 255 )
            {
                  $this->vars["error"][] = "Maximum 255 characters are allowed for First Name.";
            }
            elseif(strlen($this->getVar("lastName")) > 255 )
            {
                  $this->vars["error"][] = "Maximum 255 characters are allowed for Last Name.";
            }
            elseif(strlen($this->getVar("phone")) > 30 )
            {
                  $this->vars["error"][] = "Maximum 30 characters are allowed for Phone.";
            }
            elseif(strlen($this->getVar("fax")) > 30 )
            {
                  $this->vars["error"][] = "Maximum 30 characters are allowed for Fax.";
            }
            else
            {
                if($result["is_success"]) {
                    ...
                    if(empty($password)) $this->vars["error"][] = "The password field cannot be empty";
                    if($password != $passwordConfirm) $this->vars["error"][] = "Your passwords do not match.";
            ...
            if($result["is_success"]) {
                ...
                elseif(strlen($this->getVar("title")) > 60 ) $this->vars["error"][] = "Maximum 60 characters are allowed for Title. (ex. Ms. or Mr.)";
                elseif(strlen($this->getVar("firstName")) > 255 ) $this->vars["error"][] = "Maximum 255 characters are allowed for First Name.";
                elseif(strlen($this->getVar("lastName")) > 255 ) $this->vars["error"][] = "Maximum 255 characters are allowed for Last Name.";
                elseif(strlen($this->getVar("phone")) > 30 ) $this->vars["error"][] = "Maximum 30 characters are allowed for Phone.";
                elseif(strlen($this->getVar("fax")) > 30 ) $this->vars["error"][] = "Maximum 30 characters are allowed for Fax.";
        ...
        if($this->getVar("new")) {
            ...
            else
            {
                ...
                if($result["is_success"]) {
                    ...
                    if(empty($forwards)) $this->vars["error"][] = "You must provide valid email address(es) to forward to.";
                    else {
    ...
    public function saveAlias($params) {
        ...
        $alias = $this->getVar("alias")."@".$this->getVar("alias_domain");
        if(empty($alias)) {
            $this->vars["error"][] = "The name field cannot be empty.";
        }
        elseif(!filter_var($alias, FILTER_VALIDATE_EMAIL)){
          $this->vars["error"][] = "Invalid Email format. (ex. john.doe or john_doe or johndoe)"; 
        } else {
    ...
    public function saveWorkgroup($params) {
        $success = false;
        $this->vars["error"][] = "Managing Workgroups is not supported with this module.";
